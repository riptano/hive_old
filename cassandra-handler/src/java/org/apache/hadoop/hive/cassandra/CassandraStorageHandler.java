...
public class CassandraStorageHandler
  ...
  public void configureTableJobProperties(TableDesc tableDesc, Map<String, String> jobProperties) {
    ...
    {
      try {
        Set<String> columnNames = CassandraPushdownPredicate.getIndexedColumnNames(host, Integer.parseInt(port), keyspace, columnFamily);
        jobProperties.put(AbstractColumnSerDe.CASSANDRA_INDEXED_COLUMNS,
                          Joiner.on(AbstractColumnSerDe.DELIMITER).join(columnNames));
  ...
  public DecomposedPredicate decomposePredicate( JobConf jobConf, Deserializer deserializer, ExprNodeDesc predicate) {
    try {
      ...
      Set<String> columnNames = CassandraPushdownPredicate.getIndexedColumnNames(host, port, ksName, cfName);
